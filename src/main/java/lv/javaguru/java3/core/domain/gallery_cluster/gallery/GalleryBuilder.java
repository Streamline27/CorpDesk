...
public class GalleryBuilder {
    ...
    public GalleryBuilder withUserId(long userId) {
        this.userId = userId;
        return this;
    }
    ...
    public GalleryBuilder but() {
        return aGallery().withUserId(userId).withAllowRate(allowRate).withAllowRateIcons(allowRateIcons).withLabel(label).withDescription(description).withIsActive(isActive);
    }
